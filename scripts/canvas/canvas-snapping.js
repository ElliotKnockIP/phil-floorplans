...
export function initCanvasSnapping(fabricCanvas) {
  ...
  const handleRiskSnapping = (obj) => {
    ...
    // Update associated text
    const risk = window.risks?.find((r) => r.polygon === obj);
    if (risk?.text && fabricCanvas.getObjects().includes(risk.text)) {
      const newCenter = obj.getCenterPoint();
      risk.text.set({
        left: newCenter.x + (risk.text.offsetX || 0),
        top: newCenter.y + (risk.text.offsetY || 0),
      });
      risk.text.setCoords();
    }
  ...
  const setupEventHandlers = () => {
    fabricCanvas.on("object:moving", (e) => {
      ...
      // Risk snapping (always enabled)
      if (obj?.type === "polygon" && obj.class === "risk-polygon") {
        handleRiskSnapping(obj);
        return;
      }
