export function initCanvasSnapping(fabricCanvas) {
  ...
  // Show snap lines based on snap type
  const showSnapLines = (snapPoint, type, bounds) => {
    ...
    lineConfigs[type]?.forEach((coords) => {
      addSnapLine(createSnapLine(...coords));
    });
  ...
  // Check snapping for different point types
  ...
  // Zone snapping functions
  const isNearOriginal = (currentCenter, originalCenter) => calculateDistance(currentCenter, originalCenter) <= ZONE_SNAP_THRESHOLD;

  const handleZoneSnapping = (obj) => {
    if (!obj.originalCenter) return false;

    const currentCenter = obj.getCenterPoint();
    if (!isNearOriginal(currentCenter, obj.originalCenter)) return false;
