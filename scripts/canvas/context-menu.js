...
export function initContextMenu(fabricCanvas) {
  ...
  canvasContainer.addEventListener("contextmenu", (e) => {
    // Store the event for split functionality
    ...
    if (e.clientX >= rect.left && e.clientX <= rect.right && e.clientY >= rect.top && e.clientY <= rect.bottom) {
      ...
      if (target) {
        ...
        console.log('Context menu target:', target.type, target);
  ...
  fabricCanvas.on("mouse:down", (opt) => {
    ...
    if (e.button === 2 && opt.target) {
      ...
      console.log('Mouse down context menu target:', opt.target.type, opt.target);
      
      // Show/hide buttons based on target type
      if (opt.target.type === "group" && opt.target.deviceType && opt.target.deviceType !== "title-block") {
        copyBtn.style.display = "block";
        splitBtn.style.display = "none";
      } else if (opt.target.type === "line" && (opt.target.isNetworkConnection || opt.target.isConnectionSegment)) {
        copyBtn.style.display = "none";
        splitBtn.style.display = "block";
      } else {
        copyBtn.style.display = "none";
  ...
  splitBtn.addEventListener("click", () => {
    ...
    console.log('Split button clicked for:', currentTarget.type, currentTarget);
    
    // Get the pointer position from the stored context menu event
    let pointer;
    if (window.lastContextMenuEvent) {
      pointer = fabricCanvas.getPointer(window.lastContextMenuEvent);
    } else {
      // Fallback to center of the line
    ...
    console.log('Split at pointer:', pointer);
  ...
  window._fabricContextMenu = {
    showMenu: (t, x, y) => {
      ...
      if (t && t.type === "group" && t.deviceType && t.deviceType !== "title-block") {
        copyBtn.style.display = "block";
        splitBtn.style.display = "none";
      } else if (t && (t.isNetworkConnection || t.isConnectionSegment)) {
        copyBtn.style.display = "none";
        splitBtn.style.display = "block";
      } else {
        copyBtn.style.display = "none";
