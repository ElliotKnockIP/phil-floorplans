...
const perItemContainers = { zones: null, rooms: null, risks: null };
...
const categorizeObject = (obj) => {
  ...
  } else if (["line", "rect", "circle", "group", "path", "arrow", "textbox"].includes(obj.type) || (obj.type === "i-text" && obj.class !== "zone-text" && obj.class !== "room-text" && obj.class !== "risk-text")) {
    layers.drawings.objects.push(obj);
  ...
  Object.keys(layers).forEach((layerName) => {
    ...
    layers[layerName].objects.forEach((obj) => {
      ...
      if (layerName === "zones" && obj.type === "polygon") {
        ...
        obj.set({
          fill: setColorAlpha(obj.fill, 0.35 * objectEffectiveOpacity),
          stroke: setColorAlpha(obj.stroke, objectEffectiveOpacity),
        });
...
// Ensure the per-item UI containers for zones, rooms, and risks exist in the layer menu
const ensurePerItemContainers = () => {
  ...
  ["zones", "rooms", "risks"].forEach((name) => {
    ...
    label.textContent = name === "zones" ? "Individual Zone Controls" : name === "rooms" ? "Individual Room Controls" : "Individual Risk Controls";
...
// Render list of individual zones, rooms, or risks with visibility/opacity controls
const renderLayerItems = (layerName) => {
  ...
  const items = (layerName === "zones" ? window.zones : layerName === "rooms" ? window.rooms : window.risks) || [];
  ...
  items.forEach((item, idx) => {
    ...
    const name = layerName === "zones" 
      ? polygon.zoneName || `Zone ${idx + 1}` 
      : layerName === "rooms" 
        ? polygon.roomName || item?.roomName || `Room ${idx + 1}`
        : polygon.riskName || item?.riskName || `Risk ${idx + 1}`;
