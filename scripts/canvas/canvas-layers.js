...
function ensurePerItemContainers() {
  ["zones", "rooms"].forEach((name) => {
    ...
    // Place containers under their respective layer sliders, not under access layer
    const layerSlider = document.getElementById(`${name}-layer-opacity-slider`);
    if (!layerSlider) return;

    // Create container only once and insert after the respective layer slider
    const container = document.createElement("div");
    container.id = `${name}-layer-items`;
    container.style.margin = "8px 0 0 0"; // Remove bottom margin to avoid double spacing
    ...
    // Insert after the respective layer slider
    layerSlider.parentNode.insertBefore(container, layerSlider.nextSibling);
