...
const SYSTEM_LAYERS = ["cctv", "intruder", "fire", "access", "custom"];
...
const categorizeObject = (obj) => {
  ...
  if (obj.type === "group" && obj.deviceType) {
    // If an object has coverageConfig it's a camera; force it into CCTV layer even for custom icons
    const category = obj.coverageConfig ? "cctv" : findDeviceCategory(obj.deviceType);
...
// Ensure per-item containers exist within the layer controls submenu
function ensurePerItemContainers() {
  // Check if containers already exist and are properly positioned
  ...
  ["zones", "rooms"].forEach((name) => {
    ...
    container.className = "mb-3"; // Match the styling of other layer controls
    ...
    // Add a label that matches the layer control styling
    ...
    // Insert at the end of the layer controls submenu
