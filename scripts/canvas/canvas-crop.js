export function initCanvasCrop(fabricCanvas, subSidebar) {
  ...
  function resetCropper() {
    ...
    if (cropPreviewImage) {
      ...
      // IMPORTANT: clear previous error handler to avoid false alert when src is cleared
      cropPreviewImage.onerror = null;
    }
  }
  ...
  function showCropModal() {
    ...
    if (cropPreviewImage) {
      cropPreviewImage.onload = () => {
        // Successful load; prevent future false error alerts
        cropPreviewImage.onerror = null;
        initializeCropper();
        cropPreviewImage.onload = null;
      };
      ...
      cropPreviewImage.onerror = () => {
        // Suppress alert if we intentionally reset/cleared or no data URL
        if (!currentCanvasDataURL) {
          console.warn("Screenshot image load aborted or empty data URL; ignoring.");
        } else {
