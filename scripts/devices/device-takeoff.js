export class DeviceTakeoffGenerator {
  ...
  generateCSV() {
    ...
    let csv = "#,Floor,Device Name,Location,Mounted,Zone,Room,Part No.,Stock No.,Qty\n";
    let deviceCounter = 1;
...
function createFiltersPanel(takeoffGenerator) {
  ...
  const renderStringOptions = (list, selected) =>
    list
      .map((opt) => `<option value="${escapeAttr(opt)}" ${selected?.includes(opt) ? "selected" : ""}>${escapeHtml(opt)}</option>`)
      .join("");

  const renderCategoryOptions = (list, selected) =>
    list
      .map(({ value, label }) => `<option value="${escapeAttr(value)}" ${selected?.includes(value) ? "selected" : ""}>${escapeHtml(label)}</option>`)
      .join("");
      ...
      <div class="card-body">
        <div class="row g-2 align-items-end">
          <div class="col-12 col-md-3">
            ...
            <select id="filter-systems" class="form-select form-select-sm" multiple>
              ${renderCategoryOptions(options.deviceTypes, active.deviceTypes)}
            </select>
          </div>
          <div class="col-12 col-md-3">
            <label class="form-label mb-1">Filter by Floors</label>
            <select id="filter-floors" class="form-select form-select-sm" multiple>
              ${renderStringOptions(options.floors, active.floors)}
            </select>
          </div>
          <div class="col-12 col-md-3">
            <label class="form-label mb-1">Filter by Zones</label>
            <select id="filter-zones" class="form-select form-select-sm" multiple>
              ${renderStringOptions(options.zones, active.zones)}
            </select>
          </div>
          <div class="col-12 col-md-3">
            <label class="form-label mb-1">Filter by Rooms</label>
            <select id="filter-rooms" class="form-select form-select-sm" multiple>
              ${renderStringOptions(options.rooms, active.rooms)}
            </select>
          </div>
          <div class="col-12 d-flex gap-2 mt-2">
            <button id="apply-takeoff-filters" class="btn btn-sm btn-outline-secondary">Apply Filters</button>
            <button id="reset-takeoff-filters" class="btn btn-sm btn-outline-secondary">Clear Filters</button>
          </div>
        </div>
      </div>
    </div>
...
function bindFilterEvents(takeoffGenerator) {
  ...
  if (resetBtn)
    resetBtn.addEventListener("click", () => {
      takeoffGenerator.setFilters({});
      // Re-render filters panel to clear selections
      const container = document.getElementById("takeoff-summary-cards");
      if (container) container.innerHTML = createFiltersPanel(takeoffGenerator);
  ...
  // Also auto-apply on change for quicker UX
