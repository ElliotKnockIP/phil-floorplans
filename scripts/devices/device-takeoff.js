export class DeviceTakeoffGenerator {
  ...
  generateCSV() {
    ...
    let csv = "#,Floor,Device Name,Location,Fitting Position,Zone,Room,Part No.,Stock No.,Qty\n";
    let deviceCounter = 1;
...
function setupModalEventListeners(takeoffGenerator, { rebind = false } = {}) {
  ...
  document.getElementById("export-takeoff-csv").addEventListener("click", () => {
    ...
    if (!csv || csv.trim() === "" || csv.trim() === "#,Floor,Device Name,Location,Fitting Position,Zone,Room,Part No.,Stock No.,Qty") {
      alert("No devices found to export");
      return;
    }
    ...
    if (fabricCanvas) {
      ...
      cameraDevices.forEach((device) => {
        if (device.coverageArea && device.coverageConfig?.visible) {
          ...
          // Rather than relying on the background object's index (which can change
          // during floor loads), remove and reinsert the coverage immediately
          // before the device so it will always sit under the device but above
          // the background (which should be at the very back).
