...
const normalize = (x, y) => { const len = Math.hypot(x, y) || 1; return { x: x/len, y: y/len }; };
...
const lineIntersect = (p1, p2, p3, p4) => {
  ...
  return (ua >= 0 && ua <= 1 && ub >= 0 && ub <= 1) ? 
    { x: p1.x + ua * (p2.x - p1.x), y: p1.y + ua * (p2.y - p1.y) } : null;
};
...
const calculateIconPositions = (camera, angleSpan, isFullCircle, center) => {
  ...
  const leftRad = fabric.util.degreesToRadians(isFullCircle || isSmallAngle ? 
    (camera.coverageConfig.startAngle - 5 + 360) % 360 : camera.coverageConfig.startAngle);
  const rightRad = fabric.util.degreesToRadians(isFullCircle || isSmallAngle ? 
    (camera.coverageConfig.startAngle + 5) % 360 : camera.coverageConfig.endAngle);
...
const updateIconPosition = (icon, camera, center, angle, radius, iconScale, shouldShow, iconType) => {
  ...
  if (iconType === 'left') {
    ...
    const isSmallAngle = angleSpan <= 5;
    iconAngle = isFullCircle || isSmallAngle ? 
      (camera.coverageConfig.startAngle + 5 + 90) % 360 : 
      camera.coverageConfig.endAngle + 90;
...
function createOrUpdateCoverageArea(fabricCanvas, cameraIcon) {
  ...
  // Full initialization
  let isResizingLeft = false, isResizingRight = false, isRotating = false;
  let initialMouseAngle = 0, initialStartAngle = 0, initialEndAngle = 0;
  ...
  let leftResizeIcon = null, rightResizeIcon = null, rotateResizeIcon = null, coverageArea = null;
  
  const commonProps = {
    stroke: "black", strokeWidth: 1, originX: "left", originY: "top",
    hasControls: false, hasBorders: false, selectable: false, evented: false,
    hoverCursor: "default", lockMovementX: true, lockMovementY: true,
  ...
  if (!leftResizeIcon || !rightResizeIcon || !rotateResizeIcon) {
    const iconConfig = {
      scaleX: 0.05, scaleY: 0.05, originX: "center", originY: "center",
      hasControls: false, hasBorders: false, selectable: false, evented: true,
      visible: false, opacity: layers.devices.opacity, isResizeIcon: true,
      perPixelTargetFind: false, hoverCursor: "pointer"
    };
    ...
    iconUrls.forEach(({ url, cursor, prop }) => {
      fabric.Image.fromURL(url, (icon) => {
        ...
        icon.on("mousedown", (opt) => {
          ...
          const wallObjects = fabricCanvas.getObjects().filter(obj => 
            (obj.type === "line" && !obj.deviceType && !obj.isResizeIcon) || 
            (obj.type === "circle" && obj.isWallCircle)
          );
