...
export function initDragDropDevices(fabricCanvas) {
  ...
  canvasContainer.addEventListener("drop", (e) => {
    ...
    fabric.Image.fromURL(
      ...
      (img) => {
        ...
        // Initialize complete device indicator
        setTimeout(() => {
  ...
  document.addEventListener("keydown", (e) => {
    // Check if the active element is any input, textarea, or select element
    ...
    // If an input is focused, don't delete the device
    if (isInputFocused) {
      return; // Let the input handle the keypress naturally
    }
