...
export function handleObjectDeletion(fabricCanvas, activeObject) {
  ...
  // Handle network connections and split points
  if (activeObject.isNetworkConnection || activeObject.isConnectionSegment || activeObject.isNetworkSplitPoint) {
    if (window.topologyManager) {
      // Remove the connection and all its segments and split points
...
export function setupColorPicker(fabricCanvas) {
  ...
  function updateColorPicker(e) {
    ...
    // If a network connection segment or split point is selected, reflect the underlying connection's base color
    try {
  ...
  picker.addEventListener("input", () => {
    ...
    const newColor = picker.value;
     ...
     // Special handling: if a connection segment or split point is selected, update the entire connection base color
    try {
      if (active.isConnectionSegment || active.isNetworkSplitPoint) {
        ...
        if (tm && active.connectionId && tm.connections && tm.connections.get) {
          ...
          if (conn) {
            ...
            // Re-render this connection to apply color to all segments
...
// Set up a dedicated Text Colour picker for changing only text colours in drawing tools
export function setupTextColorPicker(fabricCanvas) {
  const picker = document.getElementById("drawing-text-color-picker");
  if (!picker) return;

  // Optional quick-select colour icons within this section
  const colorIcons = Array.from(document.querySelectorAll(".drawing-text-colour .colour-icon"));

  // Update picker when selection changes to reflect current text colour
  function updateTextPicker(e) {
    const obj = e.selected?.[0];
    if (!obj) return;

    // Determine text object in selection or inside known groups
    let textObj = null;

    if (obj.type === "i-text" || obj.type === "textbox" || obj.type === "text") {
      textObj = obj;
    } else if (obj.type === "group") {
      // Measurement group contains line + i-text; buildingFront contains triangle + text
  ...
  // Apply selected text colour to active text (or text within supported groups)
  const applyTextColor = (hex) => {
    ...
    if (active.type === "i-text" || active.type === "textbox" || active.type === "text") {
      ...
      // Apply to first text-like child if present
  ...
  // Handle direct colour input changes
  picker.addEventListener("input", () => applyTextColor(picker.value));

  // Handle quick colour icon clicks
