...
class PolygonDrawer {
  ...
  createZone(area, center) {
    ...
    // Add both objects but suppress automatic undo tracking (they are created together)
    const undoSystem = window.undoSystem;
    const prevExecuting = undoSystem ? undoSystem.isExecutingCommand : false;
    ...
    if (undoSystem) {
      ...
      const addPolygon = new window.UndoCommands.AddCommand(this.fabricCanvas, polygon, [text]);
      undoSystem.addToStack(addPolygon);
    }
  }
