...
document.addEventListener("DOMContentLoaded", () => {
    ...
    deviceLabelInput.addEventListener("input", (e) => {
      if (currentTextObject) {
        ...
        if (!currentTextObject._isHidden && currentTextObject.canvas) {
          // If text is empty, hide the background regardless of toggle state
          if (newText.trim() === "") {
            setMultipleObjectProperties(currentTextObject, { text: newText, backgroundColor: "transparent" }, currentTextObject.canvas);
          } else {
            // If text has content, respect the background toggle setting
            const shouldShowBackground = deviceLabelBackgroundToggle ? deviceLabelBackgroundToggle.checked : true;
            const bgColor = shouldShowBackground ? (currentTextObject.backgroundColor === "transparent" ? "rgba(20, 18, 18, 0.8)" : currentTextObject.backgroundColor) : "transparent";
            setMultipleObjectProperties(currentTextObject, { text: newText, backgroundColor: bgColor }, currentTextObject.canvas);
          }
        }
  ...
  if (partNumberInput) {
    ...
    preventEventPropagation(partNumberInput, ["mousedown", "keydown", "keyup"]);
  }
  ...
  if (deviceLocationInput) {
    ...
    preventEventPropagation(deviceLocationInput, ["mousedown", "keydown", "keyup"]);
  }

  // Sets up mounted position select handler
  if (fittingPositionsInput) {
  ...
  if (stockNumberInput) {
    ...
    preventEventPropagation(stockNumberInput, ["mousedown", "keydown", "keyup"]);
  }
