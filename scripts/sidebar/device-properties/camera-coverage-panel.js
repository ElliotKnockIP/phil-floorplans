...
import { drawSideView } from "../../devices/camera/camera-side-view.js";
import { calculateCameraPhysics, applyCameraPhysics } from "../../devices/camera/camera-physics.js";
...
export function initCameraCoveragePanel() {
  ...
  function updateRadiusFromHeightAndTilt(activeObject) {
  ...
  function initCameraControls(fabricCanvas) {
    ...
    if (opacitySlider && opacityInput) {
      ...
      opacityInput.addEventListener("input", () => {
        ...
        if (!isNaN(percentValue)) {
          ...
          const activeObject = fabricCanvas.getActiveObject();
          if (activeObject && activeObject.coverageConfig) {
            updateCoverageOpacity(activeObject, decimalValue);
          }
        }
      });
    ...
    document.addEventListener("camera-specs-changed", (e) => {
      if (currentGroup && currentGroup === e.detail.group) {
        updateRadiusFromHeightAndTilt(currentGroup);
  ...
  return {
    ...
    updateCameraCoveragePanel: (group) => {
      ...
      if (group && group.coverageConfig) {
        ...
        // Get current radius (distance) and minRange (deadZone) from stored values
        ...
        const deadZone = (group.coverageConfig.minRange || 0) / pixelsPerMeter;
