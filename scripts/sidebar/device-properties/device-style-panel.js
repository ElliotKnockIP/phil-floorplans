...
export function initDeviceStylePanel() {
  ...
  const deviceTextBgColorIcons = document.querySelectorAll(
    ".device-background-text-colour .colour-icon"
  );
  ...
  panel.findInGroup = function (predicate) {
    return panel.currentGroup && typeof panel.currentGroup.getObjects === "function"
      ? panel.currentGroup.getObjects().find(predicate)
      : null;
  };
  ...
  panel.updateIconColor = function (color) {
    if (
      panel.currentGroup &&
      panel.currentGroup.canvas &&
      typeof panel.currentGroup.getObjects === "function"
    ) {
  ...
  panel.setLabelTextSafely = function (textObj, newText) {
    ...
    if (!textObj._isHidden && textObj.canvas) {
      ...
      setMultipleObjectProperties(
        textObj,
        { text: newText, backgroundColor: bgColor },
        textObj.canvas
      );
    }
  };
  ...
  panel.updateDeviceTextColor = function (color) {
    if (
      panel.currentTextObject &&
      panel.currentTextObject.canvas &&
      !panel.currentTextObject._isHidden
    ) {
      setMultipleObjectProperties(
        panel.currentTextObject,
        { fill: color },
    ...
    if (
      panel.currentGroup &&
      panel.currentGroup.deviceType === "text-device" &&
      panel.currentGroup.canvas
    ) {
  ...
  (function setupControls() {
    ...
    createSliderInputSync(iconSizeSlider, iconSizeInput, (size) => panel.updateIconSize(size), {
      min: 10,
      max: 100,
      step: 1,
    ...
    setupColorControls(deviceTextColorPicker, deviceTextColorIcons, (color) =>
      panel.updateDeviceTextColor(color)
    );
    setupColorControls(deviceTextBgColorPicker, deviceTextBgColorIcons, (color) =>
      panel.updateDeviceTextBackgroundColor(color)
    );
    createSliderInputSync(
      iconRotationSlider,
      iconRotationInput,
      (angle) => panel.updateIconRotation(angle),
      { min: 0, max: 360, step: 1, format: (v) => v.toFixed(0) + "°" }
    );
          ...
          fabric.Image.fromURL(
            ...
            (newImg) => {
              ...
              if (isCamera && !panel.currentGroup.coverageConfig) {
                ...
                ["coverageArea", "leftResizeIcon", "rightResizeIcon", "rotateResizeIcon"].forEach(
                  (prop) => {
    ...
    if (deviceLabelInput) {
      ...
      deviceLabelInput.addEventListener("input", (e) => {
        if (panel.currentTextObject) {
          ...
          const isTextDevice =
            panel.currentGroup && panel.currentGroup.deviceType === "text-device";
          ...
          if (isTextDevice && panel.currentGroup) {
            ...
            const rebuildTextDeviceGroup = (textValue) => {
              ...
              newGroup.on("selected", () => {
                ...
                window.showDeviceProperties &&
                  window.showDeviceProperties("text-device", newGroup.textObject, newGroup);
              });
              newGroup.on(
                "deselected",
  ...
  panel.updateIconPanel = function (group, textObject, isTextDevice) {
    ...
    if (isTextDevice && group) {
      ...
      if (shapeObj && iconColorPicker)
        panel.syncPickerFromFill(iconColorPicker, shapeObj.fill, "#000000");
      ...
      if (panel.currentGroup && typeof panel.currentGroup.getObjects === "function") {
        ...
        if (circleObj && iconColorPicker)
          panel.syncPickerFromFill(iconColorPicker, circleObj.fill, "#000000");
      }
      ...
      if (group && typeof group.getObjects === "function" && !isTextDevice) {
        if (iconSizeSlider && iconSizeInput && group.scaleFactor !== undefined) {
          const currentSize = Math.max(
            1,
            Math.min(100, Math.round(group.scaleFactor * DEFAULT_DEVICE_ICON_SIZE))
          );
          ...
          if (circleObj) {
            const currentSize = Math.max(
              1,
              Math.min(100, Math.round((circleObj.radius / 20) * DEFAULT_DEVICE_ICON_SIZE))
            );
            iconSizeSlider.value = currentSize;
            iconSizeInput.textContent = currentSize.toFixed(0) + "px";
            updateSliderTrack(iconSizeSlider, currentSize, 1, 100);
            ...
            const defaultSize = Math.max(
              1,
              Math.min(100, window.defaultDeviceIconSize || DEFAULT_DEVICE_ICON_SIZE)
            );
  ...
  panel.updateDeviceLabelPanel = function (textObject, group, isTextDevice) {
    ...
    if (textObject && deviceLabelInput) {
      ...
      if (deviceTextColorPicker && textObject.fill && !textObject._isHidden)
        deviceTextColorPicker.value = getHexFromFill(textObject.fill);
      if (deviceTextBgColorPicker && textObject.backgroundColor && !textObject._isHidden)
        deviceTextBgColorPicker.value = getHexFromFill(textObject.backgroundColor) || "#141212";
      else if (deviceTextBgColorPicker) deviceTextBgColorPicker.value = "#141212";

      // If label is hidden, make sure it stays off the canvas
      if (
        textObject._isHidden &&
        textObject.canvas &&
        textObject.canvas.getObjects().includes(textObject)
      ) {
        try {
    ...
    if (isTextDevice && group) {
      ...
      if (textObj && deviceTextColorPicker)
        panel.syncPickerFromFill(deviceTextColorPicker, textObj.fill, "#ffffff");
  ...
  return {
    ...
    updateIconPanel: (group, textObject, isTextDevice) =>
      panel.updateIconPanel(group, textObject, isTextDevice),
    updateDeviceLabelPanel: (textObject, group, isTextDevice) =>
      panel.updateDeviceLabelPanel(textObject, group, isTextDevice),
