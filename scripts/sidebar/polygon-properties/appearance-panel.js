...
export function initAppearancePanel(detailsPanelInstance) {
  ...
  // Risk controls
  const riskTextSizeInput = document.getElementById("risk-text-size-input");
  const riskTextSizeSlider = document.getElementById("risk-text-size-slider");
  const riskColorPicker = document.getElementById("risk-color-picker");
  const riskColorIcons = document.querySelectorAll(".change-risk-colour .colour-icon");
  const riskTextColorPicker = document.getElementById("risk-text-color-picker");
  ...
  function updateZoneColor(color) {
    ...
    updateControlPointColors(currentPolygon);
  }
  ...
  function updateRoomColor(color) {
    ...
    updateControlPointColors(currentRoomPolygon);
  }
  ...
  // Updates risk text display when toggles change
  function updateRiskText() {
    const { currentRisk, currentRiskPolygon, currentRiskText } = detailsPanelInstance.getCurrentRisk();
    if (!currentRiskPolygon || !currentRiskText || !currentRiskPolygon.canvas) return;
  ...
  function updateRiskColor(color) {
    const { currentRisk, currentRiskPolygon, currentRiskText } = detailsPanelInstance.getCurrentRisk();
    if (!currentRiskPolygon || !currentRiskText || !currentRisk) return;
    // Convert hex to rgba for fill (0.35 opacity for risks - more prominent than zones)
    const r = parseInt(color.slice(1, 3), 16);
    const g = parseInt(color.slice(3, 5), 16);
    const b = parseInt(color.slice(5, 7), 16);
    const fillColor = `rgba(${r}, ${g}, ${b}, 0.35)`;
    setMultipleObjectProperties(currentRiskPolygon, { fill: fillColor, stroke: color }, currentRiskPolygon.canvas);
    setMultipleObjectProperties(currentRiskText, { fill: color, cursorColor: color });
    currentRisk.riskColor = color;
    updateControlPointColors(currentRiskPolygon);
  }

  // Updates risk text color
  function updateRiskTextColor(color) {
    const { currentRiskText } = detailsPanelInstance.getCurrentRisk();
    if (currentRiskText) {
      setMultipleObjectProperties(currentRiskText, { fill: color, cursorColor: color });
    }
  }
  ...
  // Sets up risk text size slider
  createSliderInputSync(
    riskTextSizeSlider,
    riskTextSizeInput,
    (size) => {
      const { currentRiskPolygon, currentRiskText } = detailsPanelInstance.getCurrentRisk();
      if (currentRiskPolygon && currentRiskText && currentRiskPolygon.canvas) {
        setMultipleObjectProperties(currentRiskText, { fontSize: size }, currentRiskPolygon.canvas);
      }
    },
    { min: 10, max: 30, step: 1, format: (value) => value + "px" }
  );
  ...
  return {
    ...
    updateAppearancePanel: (type, polygon, textObject, zoneOrRoom) => {
      if (type === "zone") {
        ...
        if (riskTextSizeInput && riskTextSizeSlider && textObject) {
          let textSizeValue = textObject.fontSize || 13;
          if (isNaN(textSizeValue) || textSizeValue < 10 || textSizeValue > 30) textSizeValue = 13;
          riskTextSizeInput.textContent = textSizeValue + "px";
          riskTextSizeSlider.value = textSizeValue;
          textObject.fontSize = textSizeValue;
          updateSliderTrack(riskTextSizeSlider, textSizeValue, 10, 30);
        }
