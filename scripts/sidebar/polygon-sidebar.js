...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const updateTextColor = (textObject, color) => textObject && setMultipleObjectProperties(textObject, { fill: color, cursorColor: color });
  ...
  function updateRoomTextColor(color) { updateTextColor(currentRoomText, color); }
  ...
  wrapGlobalFunction("showDeviceProperties", (deviceType, textObject, polygon, fourthParam) => {
    ...
    if (deviceType === "zone-polygon") {
      ...
      if (zoneNumberInput) {
        // zoneNumber stored on polygon so it survives serialization
  ...
  if (zoneNumberInput) {
    zoneNumberInput.addEventListener("input", (e) => {
      if (currentPolygon && currentTextObject && currentPolygon.canvas) {
        // Keep as string to allow non-numeric identifiers; trim whitespace
        const v = (e.target.value || "").trim();
        currentPolygon.zoneNumber = v;
      }
    });
    // Prevent keyboard events (delete/backspace) from bubbling to canvas
