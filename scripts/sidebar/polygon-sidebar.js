...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateZoneText() {
    ...
    if (lines.length === 0) {
      // No items selected to display: hide/remove text from canvas
      if (canvas.getObjects().includes(currentTextObject)) {
        try {
          canvas.remove(currentTextObject);
        } catch (_) {}
      }
      currentTextObject.visible = false;
      currentTextObject._isHidden = true;
      safeCanvasRender(canvas);
      return;
    }

    // Ensure text is on canvas and visible
    if (!canvas.getObjects().includes(currentTextObject)) {
      try {
    ...
    currentTextObject._isHidden = false;
    setMultipleObjectProperties(currentTextObject, { text: lines.join("\n"), visible: true }, canvas);
    try {
      currentTextObject.bringToFront && currentTextObject.bringToFront();
    } catch (_) {}
  }
  ...
  function updateRoomText() {
    ...
    if (lines.length === 0) {
      if (canvas.getObjects().includes(currentRoomText)) {
        try {
          canvas.remove(currentRoomText);
        } catch (_) {}
      }
      currentRoomText.visible = false;
      currentRoomText._isHidden = true;
      safeCanvasRender(canvas);
      // store computed metrics even if hidden
    ...
    if (!canvas.getObjects().includes(currentRoomText)) {
      try {
        canvas.add(currentRoomText);
      } catch (_) {}
    }
    currentRoomText.visible = true;
    currentRoomText._isHidden = false;
    setMultipleObjectProperties(currentRoomText, { text: lines.join("\n"), visible: true }, canvas);
    try {
      currentRoomText.bringToFront && currentRoomText.bringToFront();
    } catch (_) {}
