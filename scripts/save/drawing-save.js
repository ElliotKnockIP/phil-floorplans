...
class OptimizedDrawingObjectSerializer {
  ...
  serializeDrawingObjects() {
    ...
    return {
      ...
      // Persist topology with the drawing so multi-floor save/load keeps links
      topology: (window.topologyManager && typeof window.topologyManager.getConnectionsData === 'function')
        ? window.topologyManager.getConnectionsData()
        : [],
    };
  }
  ...
  serializeZone(zone, index) {
    try {
      return {
        ...
        zoneNumber: zone.polygon.zoneNumber || (zone.zoneNumber || ""),
        zoneResistanceValue: zone.polygon.zoneResistanceValue || (zone.zoneResistanceValue || ""),
  ...
  async loadDrawingObject(objectData) {
    return new Promise((resolve, reject) => {
      try {
        // Skip any legacy network connection lines that may have been serialized in older projects
        if (objectData.drawingType === 'line') {
          const stroke = objectData.properties?.stroke;
          const isConnColor = typeof stroke === 'string' && (stroke.toLowerCase() === '#2196f3' || /rgba?\(\s*33\s*,\s*150\s*,\s*243/i.test(stroke));
          if (isConnColor && objectData.properties?.selectable === false && objectData.properties?.evented === false) {
            // Drop this object; topology will re-render proper segments
            return resolve(null);
          }
        }
            ...
            if (objectData.groupType === "measurement" && objectData.measurementData) {
              try {
                ...
                const text = new fabric.IText(objectData.measurementData.text.text || "", {
                  ...
                  fontStyle: objectData.measurementData.text.fontStyle,
    ...
    for (const zoneData of zonesData) {
      try {
        const polygon = new fabric.Polygon(zoneData.polygon.points, {
          ...
          zoneNumber: zoneData.zoneNumber !== undefined ? zoneData.zoneNumber : (zoneData.polygon.zoneNumber || undefined),
          zoneResistanceValue: zoneData.zoneResistanceValue !== undefined ? zoneData.zoneResistanceValue : (zoneData.polygon.zoneResistanceValue || undefined),
          area: zoneData.area,
  ...
  reinitializeDrawingTools() {
    ...
    if (typeof setupNetworkLinkTool === "function") setupNetworkLinkTool(this.fabricCanvas);
