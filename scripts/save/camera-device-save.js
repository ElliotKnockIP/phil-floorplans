...
class CameraDeviceSerializer {
  ...
  serializeDevice(group) {
    try {
      ...
      const deviceData = {
        ...
        // Treat as camera if it's a known camera OR if it already has coverage config
        ...
        textLabel: null,
        coverageConfig: null,
        textDeviceConfig: null, // Add support for text devices
        visual: {
      ...
      // Special handling for text devices
      if (group.deviceType === "text-device" && group.textDeviceConfig) {
        ...
        // For text devices, also serialize the shape and text objects
      ...
      // Serialize text label (but not for text devices - they don't have visible labels)
      ...
      // Serialize coverage config for any device that has it (supports custom cameras)
      if (group.coverageConfig) {
        ...
        deviceData.coverageConfig = {
          ...
          opacity: baseOpacity,
          // Persist stable baseColor (rgb only) if present for accurate reconstruction
          baseColor: group.coverageConfig.baseColor || null,
          // Include edge style for save/load persistence
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        if (deviceData.deviceType === "text-device" && deviceData.textDeviceConfig) {
          ...
          group.initialLabelText = config.text; // Use text as device name
          group.scaleFactor = deviceData.scaleFactor || 1;
          
          // Set label as hidden for text devices
          ...
          // Create the hidden textObject (label) for compatibility with device properties
          ...
          labelText._parentGroup = group; // Reference back to parent for identification
          group.textObject = labelText;

          // Attach event handlers for popover
          group.on("selected", () => {
        ...
        fabric.Image.fromURL(
          ...
          (img) => {
            try {
              ...
              // Restore persistent id for topology binding
              ...
              if (deviceData.coverageConfig) {
                ...
                // Ensure baseColor is extracted if missing but fillColor exists
              ...
              // Add coverage if saved, regardless of whether device type is a known camera (supports custom cameras)
              if (deviceData.coverageConfig) {
              ...
              // Update complete device indicator after loading
              setTimeout(() => {
  ...
  createTextLabel(group, textData, scaleFactor) {
    ...
    // For text devices, never add the label to canvas
    if (isTextDevice) {
      text._isHidden = true;
      text.visible = false;
      group.labelHidden = true;
    ...
    // Handle layer categorization (but not for text devices)
    if (!isTextDevice) {
      setTimeout(() => {
  ...
  addDeviceEventHandlers(group) {
    // Remove previous handlers if we stored references (avoid duplicates when re-binding)
    ...
    group._deviceSelectedHandler = () => {
      // Respect suppression flag used during resize/rotate operations
      if (window.suppressDeviceProperties) return;
