...
const DEVICE_TYPES_BY_CATEGORY = {
  ...
  access: ['access-system.png', 'door-entry.png', 'gates.png', 'vehicle-entry.png', 'turnstiles.png', 'mobile-entry.png', 'pir.png', 'card-reader.png', 'lock.png'],
  ...
  networks: ['Series.png', 'panel-control.png', 'Sensor.png', 'interface-unit.png', 'access-panel.png', 'expander-connection.png'],
  custom: ['custom-device-icon.png', 'text-device']
};

const DEVICE_TYPE_LOOKUP = new Map(
...
export class TopologyManager {
  ...
  setupDeletionHandling() {
    ...
    this.fabricCanvas.on('object:removed', (e) => {
      ...
      if (target.isSegmentDistanceLabel || target.isConnectionCustomLabel) {
        ...
        if (target.isConnectionCustomLabel && target.customTextId) {
          ...
          if (connection && connection.properties.customTextLabels) {
            ...
            connection.properties.customTextLabels = connection.properties.customTextLabels.filter(
              t => t.id !== target.customTextId
            );
          }
        }
  ...
  emitConnectionBlocked(categoryA, categoryB) {
    ...
    document.dispatchEvent(new CustomEvent('topology:connection-blocked', {
  ...
  renderConnection(connection) {
    ...
    const existingLabels = this.fabricCanvas.getObjects().filter(obj => (
      (obj.isSegmentDistanceLabel || obj.isConnectionCustomLabel) && obj.connectionId === connection.id
    ));
    
    const labelStyles = new Map();
    existingLabels.forEach(label => {
      const key = label.isSegmentDistanceLabel ? `distance_${label.segmentIndex}` : 
                   label.isConnectionCustomLabel ? `custom_${label.customTextId || 'main'}` : 'unknown';
      labelStyles.set(key, {
        fill: label.fill,
    ...
    const toRemove = this.fabricCanvas.getObjects().filter(obj => (
      (obj.isConnectionSegment || obj.isNetworkSplitPoint || obj.isSegmentDistanceLabel || obj.isConnectionCustomLabel) && obj.connectionId === connection.id
    ));
      ...
      if (distance < minDistance) {
        ...
        if (segmentLength > 0) {
          const t = Math.max(0, Math.min(1, 
            ((point.x - segmentStart.x) * dx + (point.y - segmentStart.y) * dy) / (segmentLength * segmentLength)
          ));
  ...
  clearAllConnections() {
    ...
    const objectsToRemove = this.fabricCanvas.getObjects().filter(obj => 
      obj.isConnectionSegment || obj.isNetworkSplitPoint || obj.isSegmentDistanceLabel || obj.isConnectionCustomLabel
    );
  ...
  removeConnection(connectionId) {
    ...
    if (!connection) return;
    const toRemove = this.fabricCanvas.getObjects().filter(obj => 
      (obj.isConnectionSegment || obj.isNetworkSplitPoint || obj.isSegmentDistanceLabel || obj.isConnectionCustomLabel) && obj.connectionId === connectionId
    );
  ...
  getConnectionsData() {
    ...
    this.connections.forEach(connection => {
      ...
      if (connection.properties.customTextLabels && connection.properties.customTextLabels.length > 0) {
        data.properties.customTextLabels = JSON.parse(JSON.stringify(connection.properties.customTextLabels));
      }
  ...
  this.createConnection(device1, device2, connData.type, { skipValidation: true });
      ...
      if (connection) {
        ...
        // Ensure customTextLabels is properly restored
        if (connData.properties.customTextLabels && connData.properties.customTextLabels.length > 0) {
          connection.properties.customTextLabels = JSON.parse(JSON.stringify(connData.properties.customTextLabels));
        }
