import { distance, midpoint, pathMidpoint, positionOnPath, ratioOnPath } from "./geometry.js";
...
export class ConnectionRenderer {
  ...
  clear(connectionId) {
    const toRemove = this.canvas.getObjects().filter((obj) => obj.connectionId === connectionId && (obj.isConnectionSegment || obj.isNetworkSplitPoint || obj.isSegmentDistanceLabel || obj.isConnectionCustomLabel || obj.isChannelLabel));
  ...
  saveLabelStyles(connectionId) {
    ...
    const labels = this.canvas.getObjects().filter((obj) => obj.connectionId === connectionId && (obj.isSegmentDistanceLabel || obj.isConnectionCustomLabel || obj.isChannelLabel));

    labels.forEach((label) => {
      const key = label.isSegmentDistanceLabel ? `distance_${label.segmentIndex}` : label.isConnectionCustomLabel ? `custom_${label.customTextId || "main"}` : "channel";
      styles.set(key, {
        fill: label.fill,
  ...
  renderDistanceLabels(connection, path, pixelsPerMeter, labelStyles) {
    for (let i = 0; i < path.length - 1; i++) {
      ...
      const label = new fabric.Text(`${meters} m`, {
        ...
        backgroundColor: saved?.backgroundColor || "rgba(255, 255, 255, 0.8)",
  ...
  highlightConnection(connectionId) {
    const segments = this.canvas.getObjects().filter((obj) => obj.isConnectionSegment && obj.connectionId === connectionId);
    segments.forEach((seg) => seg.set({ ...this.styles.lineHighlight }));

    const splits = this.canvas.getObjects().filter((obj) => obj.isNetworkSplitPoint && obj.connectionId === connectionId);
